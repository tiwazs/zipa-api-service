...
@router.get("/")
async def get_unit_values(request: Request, response: Response, sigma_divider: Optional[float] = 3):
    return UnitGenerationService().generate_unit(sigma_divider)

@router.get("/single_value")
async def get_unit_value(request: Request, response: Response, start: int, end: int, sigma_divider: Optional[float] = 3):
