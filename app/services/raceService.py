...
class RaceService:
    ...
    async def get_all(self, include_traits: bool, include_units: bool) -> List[RaceDTO]:
    ...
    async def get_by_id(self, id: str, include_traits: bool, include_units: bool) -> RaceDTO:
    ...
    async def create(self, race: RaceCreateDTO) -> RaceDTO:
        ...
        return await self.get_by_id(race.id, True)
    ...
    async def add_unit(self, id: str, unit_specialization_id: str) -> RaceDTO:
        race = await self.get_by_id(id, True, True)
        if(not race): return None

        # Assign unit_specializations
        await self.race_unit_service.create({"race_id":race.id, "unit_specialization_id":unit_specialization_id})

        return await self.get_by_id(race.id, True, True)
